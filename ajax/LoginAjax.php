<?php
namespace xenice\login\ajax;

use xenice\login\Ajax;
use xenice\login\includes\ImageCaptcha;
...
class LoginAjax extends Ajax
{
    ...
    public function register()
    {
        ...
        $error = '';
		if ( $username == '' ) {
			$error .= __('Please enter username', 'xenice-login') . ' ';
		}
		elseif(!validate_username( $username ) ){
		    $error .= __('This username contains an invalid character', 'xenice-login') . ' ';
		}
		elseif (username_exists( $username ) ) {
		    $error .= __('The username has been registered', 'xenice-login') . ' ';
		}
		
        if ( $email == '' ) {
            $error .= __('Please fill in your email address', 'xenice-login') . ' ';
        }
        elseif ( ! is_email( $email ) ) {
            $error .= __('The email address is incorrect', 'xenice-login') . ' ';
        }
        elseif ( email_exists( $email ) ) {
            $error .= __('The email address has been registered', 'xenice-login') . ' ';
        }
		  
        if($_POST['password'] == ''){
            $error .= __('Please enter password', 'xenice-login') . ' ';
        }
        
        if($_POST['password'] != $_POST['repassword']){
            $error .= __('Entered passwords differ', 'xenice-login') . ' ';
        }
        
        elseif(strlen($_POST['password']) < 6){
            $error .= __('Password length should not be less than 6', 'xenice-login') . ' ';
        }
        
        if(empty($_POST['captcha']) || empty($_SESSION['xenice_captcha']) || trim(strtolower($_POST['captcha'])) != $_SESSION['xenice_captcha']){
            $error .= __('Captcha error', 'xenice-login') . ' ';
        }

        if(empty($_SESSION['xenice_captcha_email']) || $_SESSION['xenice_captcha_email'] != $email){
            $error .= __('Captcha and email do not correspond', 'xenice-login') . ' ';
        }
		...
		if($error){
			...
			if ( is_wp_error( $user_id ) ) {
				echo __("Server timeout, please try again later", 'xenice-login');
			}else{
    ...
    public function check()
    {
    	...
    	if(isset($_POST['username'])){
    		...
    		if(strlen($username) < 6){
    		    $error .= __('Username at least 6 characters', 'xenice-login') . ' ';
            }elseif ( $username == '' ) {
                $error .= __('Please enter username', 'xenice-login') . ' ';
            } elseif ( ! validate_username( $username ) ) {
                $error .= __('This username contains an invalid character', 'xenice-login') . ' ';
            ...
            elseif (!is_email( $email )) {
                $error .= __('The email format is incorrect', 'xenice-login') . ' ';
            } 
            elseif(email_exists($email)) {
                $error .= __('This email address has been registered, please change it', 'xenice-login') . ' ';
            }
    	}
    	echo $error;
    	exit;
    ...
    private function sendCaptcha($email)
    {
    	...
    	$message = __('Captcha') . ': ' .$_dscode;   
    	return wp_mail($email, __('Captcha') . '-' . get_bloginfo('name'), $message);    
    }
    ...
    public function forgetPassword()
    {
        ...
        if(empty($_POST['captcha']) || empty($arr) || $_POST['captcha'] != $arr['code'] || $arr['time'] < $time){
            echo __('The captcha is incorrect', 'xenice-login');
            exit;
        }
		...
		if(!is_email($username)) {
			...
			if(!username_exists( $user_login )){
				$error = __("Username does not exist", 'xenice-login');
			}else{
				$user_data = get_user_by('login', $username);
				$user_email = $user_data->user_email;
				if(empty($user_email)){
					$error = __("User does not set email", 'xenice-login');
				}
			}
		}else{
			$user_email = $username;
			if(!email_exists( $user_email )){
				$error = __("Email does not exist", 'xenice-login');
			}else{
		...
		if($error){
			...
			if (wp_mail($user_email, $subject, $message, $headers) ) {   
		        ...
		        echo __("Send mail failed, please try again later", 'xenice-login');
		    }
		    exit();
		}
    }
    ...
    public function resetPassword()
    {
        ...
        if($_POST['password'] != $_POST['password']){
            echo __('Entered passwords differ', 'xenice-login');
            exit;
        }
